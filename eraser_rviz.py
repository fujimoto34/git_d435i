# 消しゴムの座標をサブスクライブしてMarkerをパブリッシュ
# python3 eraser_rviz.py
...
class EraserMarkerPublisher(Node):
    def __init__(self):
        super().__init__('eraser_marker_publisher')
        self.sub = self.create_subscription(Point, 'eraser_xyz', self.callback, 10)
        self.pub_eraser = self.create_publisher(Marker, 'eraser_marker', 10)
        self.pub_eraser_point = self.create_publisher(Marker, 'eraser_point_marker', 10)
    ...
    def callback(self, msg):
        ...
        marker.color.b = 0.0
        marker.color.a = 1.0
        self.pub_eraser.publish(marker)

        # 消しゴムのマーカーの上に座標を表示
        marker = Marker()
        marker.header.frame_id = 'map'
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.ns = "eraser_point"
        marker.id = 0
        ...
        self.pub_eraser_point.publish(marker)
...
# クラスをインスタンス化
eraser_marker_publisher = EraserMarkerPublisher()

# spin()でループ処理を実行
rclpy.spin(eraser_marker_publisher)

# 終了
eraser_marker_publisher.destroy_node()
rclpy.shutdown()