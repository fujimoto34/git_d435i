# 検知した石と木材の3次元座標(グローバル座標系)をパブリッシュする(空間フィルター+時間フィルター)
...
# ROS2でPublisherを作成
rclpy.init()
node = Node("stone_wood1")
pub_stone_xyz = node.create_publisher(Point, "stone_xyz", 10)
pub_wood_xyz_A = node.create_publisher(PointStamped, "wood_xyz_A", 10)
pub_wood_xyz_B = node.create_publisher(PointStamped, "wood_xyz_B", 10)
msg_stone_xyz = Point()
msg_wood_xyz_A = PointStamped()
msg_wood_xyz_B = PointStamped()  # AとBで区別したほうが安全らしい
    ...
    while True:
        ...
        for result in results:
            for obb in result.obb:
                if int(obb.cls) == 0:  # class 0 is stone
                    ...
                    if distance > 0:
                        ...
                        msg_stone_xyz.y = y_
                        msg_stone_xyz.z = z_
                        pub_stone_xyz.publish(msg_stone_xyz)
                    else:
                        print('距離が測定できませんでした')

                if int(obb.cls) == 1:  # class 1 is wood
                    x1, y1, x2, y2 = map(int, obb.xyxy[0].tolist())  # 座標を整数に変換。x1,y1は左上、x2,y2は左下
                    # 画像データに検出ボックスを書き込む
                    cv2.rectangle(color_image, (x1, y1), (x2, y2), (255, 0, 0), 3)  # 検出ボックス描画
                    label = f"{result.names[int(obb.cls)]} {obb.conf[0]:.2f}"  # テキストを定義
                    ...
                    if distanceA > 0 and distanceB > 0:
                        ...
                        msg_wood_xyz_A.header.stamp = now_time  # 時間合わせ
                        msg_wood_xyz_A.header.frame_id = 'map'  # フレームid合わせ
                        msg_wood_xyz_A.point.x = x_A  # PointStamped型の座標代入は.pointが必要
                        msg_wood_xyz_A.point.y = y_A
                        msg_wood_xyz_A.point.z = z_A
                        pub_wood_xyz_A.publish(msg_wood_xyz_A)
                        msg_wood_xyz_B.header.stamp = now_time  # 時間合わせ
                        msg_wood_xyz_B.header.frame_id = 'map'  # フレームid合わせ
                        msg_wood_xyz_B.point.x = x_B
                        msg_wood_xyz_B.point.y = y_B
                        msg_wood_xyz_B.point.z = z_B
                        pub_wood_xyz_B.publish(msg_wood_xyz_B)
                    else:
                        print('距離が測定できませんでした')
